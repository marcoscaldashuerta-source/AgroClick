...
def asignar_pedido_delivery(request, pedido_id):
    """Asigna el pedido al repartidor y marca la entrega como asignada."""
    if not request.user.is_authenticated:
        return redirect('/accounts/login/')
...
def actualizar_estado_entrega(request, pedido_id):
    """Actualiza el estado de entrega de un pedido asignado al repartidor."""
    if not request.user.is_authenticated:
        return redirect('/accounts/login/')
    ...
    if nuevo_estado == 'en_camino' and pedido.estado_entrega not in ['asignado', 'pendiente']:
        messages.error(request, 'No puedes marcar el pedido como en camino desde su estado actual.')
        return redirect('pedidos_delivery')
    ...
    if nuevo_estado == 'entregado':
        Notificacion.objects.create(
            ...
            mensaje=f'Tu pedido N°{pedido.id} fue entregado por delivery.'
        )
        Notificacion.objects.create(
...
def pedidos_delivery(request):
    """Lista pedidos delivery disponibles y pedidos asignados al repartidor."""
    if not request.user.is_authenticated:
        return redirect('/accounts/login/')
    ...
    pedidos_disponibles = Pedido.objects.filter(
        ...
        tipo_entrega='delivery',
        repartidor__isnull=True,
    ).select_related('comprador', 'producto').order_by('-fecha_creacion')
